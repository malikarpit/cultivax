...
export default function AdminDashboardPage() {
  ...
  return (
    ...
    <div className="animate-fade-in">
      <div className="mb-6">
        <h1 className="text-2xl font-bold">{t('admin.cultivax_obsidian_admin_console')}</h1>
        <p className="text-sm text-cultivax-text-muted mt-1">{t('admin.system_overview_and_management')}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="card p-5 mb-6 border border-red-500/20 flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-red-400">{t('admin.failed_to_load_admin')}</p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
          <StatCard
            icon={Users}
            label={t('admin.label.total_users')}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <div className="card">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-semibold flex items-center gap-2">
                <BarChart3 className="w-4 h-4 text-cultivax-primary" />{t('admin.farmers_by_region')}</h3>
              <span className="text-xs text-cultivax-text-muted">{t('admin.top_5_regions')}</span>
            </div>
            {loading ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-sm text-cultivax-text-muted">{t('admin.no_regional_data_available')}</p>
              </div>
            )}
          </div>
          ...
          <div className="card mt-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-semibold flex items-center gap-2">
                <Activity className="w-4 h-4 text-cultivax-primary" />{t('admin.system_health')}</h3>
              <Link href="/admin/health" className="text-xs text-cultivax-primary font-semibold hover:underline">{t('admin.view_details')}</Link>
            </div>
            <div className="grid grid-cols-3 gap-3">
              <div className="p-3 rounded-lg bg-green-500/10 text-center">
                <CheckCircle2 className="w-5 h-5 text-green-400 mx-auto mb-1" />
                <p className="text-xs text-cultivax-text-muted">{t('admin.api')}</p>
              </div>
              <div className="p-3 rounded-lg bg-green-500/10 text-center">
                <CheckCircle2 className="w-5 h-5 text-green-400 mx-auto mb-1" />
                <p className="text-xs text-cultivax-text-muted">{t('admin.database')}</p>
              </div>
              <div className="p-3 rounded-lg bg-green-500/10 text-center">
                <CheckCircle2 className="w-5 h-5 text-green-400 mx-auto mb-1" />
                <p className="text-xs text-cultivax-text-muted">{t('admin.events')}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="card">
            <h3 className="text-base font-semibold mb-3">{t('admin.quick_actions')}</h3>
          ...
          {!loading && stats && (
            <div className="card">
              <h3 className="text-base font-semibold mb-3">{t('admin.platform_summary')}</h3>
              <div className="space-y-3 text-sm">
                <div className="flex items-center justify-between">
                  <span className="text-cultivax-text-muted">{t('admin.total_crops')}</span>
                  <span className="font-semibold text-cultivax-text-primary">
                    {stats.total_crops?.toLocaleString() ?? '0'}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-cultivax-text-muted">{t('admin.active_crops')}</span>
                  <span className="font-semibold text-cultivax-primary">
                    {stats.active_crops?.toLocaleString() ?? '0'}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-cultivax-text-muted">{t('admin.verified_providers')}</span>
                  <span className="font-semibold text-cultivax-text-primary">
                    {stats.verified_providers ?? 0} / {stats.total_providers ?? 0}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-cultivax-text-muted">{t('admin.regions_active')}</span>
                  <span className="font-semibold text-cultivax-text-primary">
                    {stats.region_breakdown?.length ?? 0}
                  </span>
                </div>
      ...
      <div className="card mt-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-base font-semibold">{t('admin.recent_admin_actions')}</h3>
          <Link href="/admin/audit" className="text-xs text-cultivax-primary font-semibold hover:underline">{t('admin.full_audit_log')}</Link>
        </div>
        <p className="text-sm text-cultivax-text-muted">{t('admin.view_the_complete_audit')}</p>
      </div>
    </div>
    </ProtectedRoute>
  );
