...
from app.services.ctis.state_machine import (BLOCKED_STATES,  # type: ignore
                                             CropStateMachine)
...
class ActionService:
    ...
    def log_action(
        ...
        last_action = (
            ...
            .order_by(ActionLog.effective_date.desc())
            .first()
        )
        ...
        # Apply direct CTIS field updates only in sanctioned context.
    ...
    def list_actions(self, crop_id: UUID, farmer_id: UUID) -> list[ActionLog]:
        ...
        return (
            self.db.query(ActionLog)
            .filter(
                ActionLog.crop_instance_id == crop_id, ActionLog.is_deleted == False
            )
            .order_by(ActionLog.effective_date.desc())
            .all()
        )
    ...
    def list_actions_paginated(
        ...
        if sort == "effective_date":
            query = query.order_by(
                ActionLog.effective_date.asc(), ActionLog.created_at.asc()
            )
        else:
            query = query.order_by(
                ActionLog.effective_date.desc(), ActionLog.created_at.desc()
            )
