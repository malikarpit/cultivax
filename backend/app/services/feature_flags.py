...
def invalidate_cache(flag_name: str = None):
    ...
    if not flag_name:
        ...
        keys_to_delete = [
            k for k in _FLAG_CACHE.keys() if k.startswith(f"{flag_name}:")
        ]
...
def is_enabled(
    ...
    try:
        ...
        flags = (
            ...
            .filter(FeatureFlag.flag_name == flag_name, FeatureFlag.is_deleted == False)
            .all()
        )
        ...
        if user_id:
            user_flag = next(
                (
                    f
                    for f in flags
                    if f.scope == "per_user" and f.scope_value == str(user_id)
                ),
                None,
            )
            if user_flag:
        ...
        if region:
            region_flag = next(
                (
                    f
                    for f in flags
                    if f.scope == "per_region" and f.scope_value == region
                ),
                None,
            )
            if region_flag:
        ...
        logging.error(
            f"Feature Flag Evaluation Error against [{flag_name}]: {repr_err}"
        )
        return default
