...
SENSITIVE_KEYS = {
    "password", "hashed_password", "token", "secret", "private_key", "access_token"
}

def strip_sensitive_payloads(payload: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
...
def create_audit_entry(
    ...
    if action in DESTRUCTIVE_ACTIONS and not reason:
        logger.error(f"Audit compliance rejected: Action '{action}' strictly requires a reason.")
        raise HTTPException(
            status_code=400, 
    ...
    audit = AdminAuditLog(
        ...
        entity_id=entity_id if isinstance(entity_id, UUID) else str(entity_id), # Some systems may pass Strings
