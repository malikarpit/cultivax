...
class SecurityEventStore:
    ...
    def add_event(
        self, event_type: str, details: str, request_id: str, path: str, identity: str
    ):
        event = {
            "timestamp": time.time(),
            "type": event_type,
...
def log_security_event(
    event_type: str, details: str, request_id: str, path: str, identity: str = "unknown"
):
    """Helper to add an event to the global store"""
    store.add_event(event_type, details, request_id, path, identity)
    logger.warning(
        f"Security Event [{event_type}]: {details} (req_id={request_id}, path={path}, id={identity})"
    )
