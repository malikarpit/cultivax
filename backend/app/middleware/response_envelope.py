...
class ResponseEnvelopeMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            if isinstance(original_data, dict) and ('detail' in original_data or 'success' in original_data):
            ...
            for k, v in response.raw_headers:
                if k.lower() == b'content-length':
                    new_raw_headers.append((b'content-length', str(len(wrapped_body)).encode('latin-1')))
                else:
