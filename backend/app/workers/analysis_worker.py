...
async def process_media_analysis(media_id: str):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Background analysis completed successfully for media {media_id}")
        finally:
            db.close()
    except Exception as e:
        logger.error(f"Background analysis failed for media {media_id}: {e}", exc_info=True)
