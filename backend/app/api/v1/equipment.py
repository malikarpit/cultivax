...
async def add_equipment(
    ...
    if not provider:
        raise HTTPException(status_code=403, detail="Provider not found or not owned by you")
...
async def list_equipment(
    ...
    total = query.count()
    items = query.order_by(Equipment.created_at.desc()).offset((page - 1) * per_page).limit(per_page).all()
