...
async def submit_review(
    ...
    if provider:
        try:
            ...
            import logging
            logging.getLogger(__name__).warning(f"TrustEngine failed, using avg fallback: {te_err}")
            avg_rating = db.query(func.avg(ServiceReview.rating)).filter(
...
async def list_reviews(
    ...
    reviews = query.order_by(ServiceReview.created_at.desc()).offset((page - 1) * per_page).limit(per_page).all()
